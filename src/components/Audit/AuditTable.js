...
import AuditInfoBar from '../utils/AuditInfoBar';
          ...
          <tbody>
            ...
            {items.map((item, idx) => (
              ...
              >
                ...
                <td className="p-3">
                  {item.response ? (
                    <span className={`inline-block px-3 py-2 rounded text-sm font-bold min-w-16 text-center ${selectedRow.table === tableKey && selectedRow.index === idx ? 'bg-white bg-opacity-20' : getResponseClass(item.response)}`}>{item.response}</span>
                  ) : (
                    <span className={`inline-block px-3 py-2 rounded text-sm text-center min-w-16 ${selectedRow.table === tableKey && selectedRow.index === idx ? 'text-gray-200' : 'text-gray-400'}`}>{selectedRow.table === tableKey && selectedRow.index === idx ? 'Sélectionner' : 'Pas de réponse'}</span>
                  )}
                </td>
                <td className="p-3">
